...
parseBnfcType (B.MPL_UIDENT_ARGS_TYPE ident _ args _) = do
  ...
  case ident' ^? name % _Name % _InternalConcTypeParser of
    Just InternalNeg -> case args' of
      ...
      err ->
        tell [_NegExpectedExactlyOneArgumentButGot # (res, err)]
          >> throwError ()
...
parseBnfcType (B.MPL_STORE_TYPE _ l (B.PROCESS_TYPE ss is os) r) = do
  ss' <- mapM parseBnfcType ss
  is' <- mapM parseBnfcType is
  os' <- mapM parseBnfcType os
  let name = _NameOcc # (_Name # "Store", toSpanLocation l r)
  return $ _TypeStoreF # (name, _TypeConcArrF # ((), ss', is', os'))
