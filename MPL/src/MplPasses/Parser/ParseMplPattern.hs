...
patternConstructorParse :: BnfcParse (IdentP, [MplPattern MplParsed]) (MplPattern MplParsed)
patternConstructorParse (identp, args) 
    | Just bv <- readMaybe (identp ^. identPNameOcc % nameOccName % coerced) :: Maybe Bool =
        if null args 
            then return $ _PBool # (identp ^. location, bv)
            else tell [_InvalidBool # identp] >> throwError ()
    | otherwise = return $ _PConstructor # ((), identp, args)
