...
typeCheckStmt (MplStmt defns wheres) = do
  ...
  rec envLcl % typeInfoEnvMap .= tagmap
      ~((defns', eqns), errpkg) <-
        fmap (first (NE.unzip . NE.fromList)) $
          runWriterT $
            typeCheckDefns $
              NE.toList defns
...
typeCheckDefn (ObjectDefn obj) =
  (((,mempty) . ObjectDefn)) <$> case obj of
    SeqObjDefn obj ->
...
typeCheckFunBody bdy@(patts, expr) = do
  ...
  let ttypep = annotateTypeTag ttype bdy
      ...
      eqn =
        TypeEqnsExist (ttypepexpr : ttypeppatts) $
          [ TypeEqnsEq
  ...
  where
    ...
    f (ETupleF cxt (t0, t1, ts)) = do
      ...
      let tuplexpr = _ETuple # (cxt, (fst t0, fst t1, map fst ts)) :: MplExpr MplRenamed
          ...
          eqn =
            TypeEqnsExist (ttypept0 : ttypept1 : ttypepts) $
              [ TypeEqnsEq
                  ( typePtoTypeVar ttypep,
                    _TypeTupleF
    ...
    f (EListF cxt ts) = do
      ...
      let ann = _EList # (cxt, map fst ts) :: MplExpr MplRenamed
          ...
          eqns =
            TypeEqnsExist [_TypeIdentT # (tp, TypeIdentTInfoTypeAnn $ TypeAnnExpr ann)] $
              [ TypeEqnsEq
                  ( typePtoTypeVar ttypep,
                    _TypeListF
                ...
                <> map
                  (TypeEqnsEq . (typePtoTypeVar tpp,) . typePtoTypeVar . uncurry annotateTypeTag)
    ...
    f (EIfF cxt mcond mthenc melsec) = do
      ...
      let ifexpr = _EIf # (cxt, fst mcond, fst mthenc, fst melsec) :: MplExpr MplRenamed
          ...
          eqn =
            TypeEqnsExist [ttypetmcond, ttypetmthenc, ttypetmelsec] $
              [ TypeEqnsEq
                  ( typePtoTypeVar ttypep,
    ...
    f (ESwitchF cxt switches) = do
      ...
      let switchexpr = _ESwitch # (cxt, switches & mapped %~ (fst *** fst)) :: MplExpr MplRenamed
          ...
          eqn =
            TypeEqnsExist (ttypestbexprs ++ ttypetexprs) $
    ...
    f expr@(EStoreF cxt (Left ident)) = do
      ttype <- guse (envLcl % typeInfoEnvTypeTag)
      ttypemap <- guse (envLcl % typeInfoEnvMap)
      ~(SymEntry tp (SymRunInfo procc)) <- zoom (envLcl % typeInfoSymTab) $ lookupSymConc ident

      arrenv <- freshInstantiateArrEnv
      let (ttypepargs, ttypeproc) =
            (`runInstantiateArrType` arrenv) $
              fromJust $
                tp ^? _SymConcCallType % to (instantiateArrType (_Just % _TypeAnnExpr # ann))
          ann = (_EStore # (cxt, Left ident)) :: MplExpr MplRenamed
          ttypep = annotateTypeTag ttype ann
          eqn =
            TypeEqnsExist
              ttypepargs
      ...
      ((phrases', phraseeqns), instt) <- fmap
        ...
        $ do
          ...
          (ttypepfroms, ttypestvar, ttypeclause) <- state $ runState $ do
            ...
            return
              ( fromJust $ traverse (instantiateTypeWithSubs (_Just % _TypeAnnExpr # foldexpr) subs) froms,
    ...
    f (EUnfoldF cxt (unfoldorigexpr, unfoldonexpr) (phrase :| phrases)) = do
      ...
      let unfoldexpr =
            EUnfold
              cxt
              unfoldorigexpr
              (over (mapped % _3 % mapped % _4) fst $ phrase :| phrases) ::
              MplExpr MplRenamed
        ...
        $ do
          ...
          (phrasesubphrases', phrasesubphraseseqns) <- fmap NE.unzip $ for phrasesubphrases $ \(cxt, ident, patts, (expr, mexpr)) -> do
            ...
            (ttypepfroms, ttypestvar, ttypeto, ttypeclause) <- state $ runState $ do
              ...
              return
                ( fromJust $ traverse (instantiateTypeWithSubs (_Just % _TypeAnnExpr # unfoldexpr) subs) froms,
            ...
            let subphrase' = (seqdef, ident, patts', expr')
                ...
                ttypepexpr = annotateTypeTag ttypeexpr expr
                subphraseeqn =
                  TypeEqnsExist (ttypepexpr : ttypepunfoldonpatt : ttypeppatts) $
                    -- the type of the overall expression is the type of
              ...
              for phrases $ \(cxt, unfoldonpatt, phrasesubphrases) -> do
                ...
                (phrasesubphrases', phrasesubphraseseqns) <- fmap NE.unzip $ for phrasesubphrases $ \(cxt, ident, patts, (expr, mexpr)) -> do
                  ...
                  (ttypepfroms, ttypestvar, ttypeto, ttypeclause) <- state $ runState $ do
                    ...
                    return
                      ( fromJust $ traverse (instantiateTypeWithSubs (_Just % _TypeAnnExpr # expr) subs) froms,
                  ...
                  let subphrase' = (seqdef, ident, patts', expr')
                      ...
                      ttypepexpr = annotateTypeTag ttypeexpr expr
                      phraseeqn =
                        TypeEqnsExist (ttypepunfoldonpatt : ttypepexpr : ttypeppatts) $
                          -- The type of the state var is the same
                          -- as the unfoldon pattern
          ...
          let nunfoldphrases' = phrase' :| phrases'
              eqns =
                TypeEqnsExist (ttypepsubphrase : ttypepsubphrases) $
    ...
    f (ECaseF cxt (caseon, mcaseon) cases) = do
      ...
      let ttypep = annotateTypeTag ttype (_ECase # (cxt, caseon, fmap (second fst) cases) :: MplExpr MplRenamed)
          ...
          eqns =
            TypeEqnsExist (ttypepcaseon : ttypeppatts ++ ttypepexprs) $
    ...
    f (EObjCallF cxt ident args) = do
      ...
      ~(SymEntry lkuptp (SymSeqPhraseCall seqdef)) <-
        fmap fromJust $
          zoom (envLcl % typeInfoSymTab) $
            lookupSymExpr ident
      ...
      (ttypeargs, (args', argseqns)) <- fmap (second unzip <<< unzip) $
        for args $
          \(_, mexpreqns) -> withFreshTypeTag mexpreqns
      ...
      let ttypep = annotateTypeTag ttype expr
          ...
          eqns =
            TypeEqnsExist (ttypesphrase ++ ttypepargs) $
              [ TypeEqnsEq
    ...
    f (ECallF cxt ident args) = do
      ...
      ~(SymEntry lkuptp (SymSeqCall seqdef)) <-
        fmap fromJust $
          zoom (envLcl % typeInfoSymTab) $
            lookupSymExpr ident
    ...
    f (ERecordF cxt phrases) = do
      ...
      ~(((ttypeppatts, ttypepexpr), (phrases', phraseseqns)), ttypepinst) <- fmap
        ...
        $ flip runStateT arrenv
        $ for phrases
        $ \(_, ident, (patts, (expr, mexpreqn))) -> do
          ~(SymEntry lkuptp seqdef) <-
            fmap fromJust $
              lift $
                zoom (envLcl % typeInfoSymTab) $
      ...
      let eqns =
            TypeEqnsExist (concat ttypeppatts <> ttypepexpr <> ttypepinst) $
              concat phraseseqns
...
typeCheckProcessBody procbdy@((patts, ins, outs), cmds) = do
  ...
  let ttypep = annotateTypeTag ttype procbdy
      ...
      eqn =
        TypeEqnsExist (ttypeppatts ++ ttypepins ++ ttypepouts) $
          -- phrase equation
...
typeCheckCmds (cmd :| rst) = do
  tell $ review _ExternalError $ case cmd of
    ...
    CRun _ (Left cxt) _ _ _ -> [review _IllegalNonLastCommand $ cxt ^. identPNameOcc % to KeyWordNameOcc]
...
typeCheckCmd cmd =
  let cmdann = _Just % _TypeAnnCmd # cmd
   in case cmd of
        CRun cxt (Left ident) seqs ins outs -> do
          ttypemap <- guse (envLcl % typeInfoEnvMap)
          ...
          let ttypespins = annotateTypeTags (map (view symEntryType) ttypesins) $ repeat cmd
              ...
              eqns =
                TypeEqnsExist (ttypepargs <> ttypespseqs <> ttypespins <> ttypespouts) $
              ...
              ins' =
                zipWith
                  ...
                  ins
                  (map (view symEntryType) ttypesins)
        ...
        r@(CRun cxt (Right expr) seqs ins outs) -> do
          ...
          (ttypeseqs, (seqs', seqseqns)) <-
            (second unzip . unzip) <$> traverse (withFreshTypeTag . typeCheckExpr) seqs
          ...
          let ttypespins = annotateTypeTags (map (view symEntryType) ttypesins) $ repeat cmd
              ttypespouts = annotateTypeTags (map (view symEntryType) ttypesouts) $ repeat cmd
              ttypespseqs = annotateTypeTags ttypeseqs $ repeat cmd
              ttypeexprs = annotateTypeTag ttypeexpr expr

              eqns =
                TypeEqnsExist ([ttypeexprs] <> ttypespseqs <> ttypespins <> ttypespouts) $
                  -- match the given types with the actual type of the process
                  [ TypeEqnsEq
                      ( typePtoTypeVar ttypeexprs,
                        _TypeStoreF
                          # ( _Just % _TypeAnnCmd # cmd,
                              _TypeConcArrF
                                # ( _Just % _TypeAnnCmd # cmd,
          ...
          return (_CRun # (Nothing, Right expr', seqs', ins', outs'), [eqns])
        CClose cxt ch -> do
          ttypemap <- guse (envLcl % typeInfoEnvMap)
        ...
        CGet cxt patt ch -> do
          ...
          let ch' = _ChIdentT # (ch, fromJust $ lookupInferredSeqTypeExpr ttypech ttypemap)
              ...
              eqn =
                TypeEqnsExist [ttypepch', ttypeppatt] $
                  [ TypeEqnsEq
                      ( typePtoTypeVar ttypepch,
                        inputOutput
                          (ch ^. polarity)
        ...
        CPut cxt expr ch -> do
          ...
          let ch' = _ChIdentT # (ch, fromJust $ lookupInferredSeqTypeExpr ttypech ttypemap)
              ...
              eqn =
                TypeEqnsExist [ttypepch', ttypepexpr] $
                  [ TypeEqnsEq
                      ( typePtoTypeVar ttypepch,
                        inputOutput
                          (ch ^. polarity)
                  ...
                  ttypepclause = fromJust $ case def of
                    ProtocolDefn phrase ->
                      phrase
                        ^. typePhraseExt
                        % to
                          ( instantiateTypeWithSubs cmdann subs
                              . typeClauseToMplType
                          )
        ...
        CHPut cxt ident ch -> do
          ...
          ~(SymEntry clauselkuptp (SymConcPhraseCall def)) <- zoom (envLcl % typeInfoSymTab) $ do
            ...
            tell $
              review _InternalError $
                maybe
                  [_CannotCallTerm # ident]
                  mempty
                  res
            return $ fromJust res
              ...
              inputOutput
                ...
                ( maybeToList $
                    def
                      ^? _ProtocolDefn
                      % to
                        ( review _HPutExpectedInputPolarityChToHaveCoprotocolButGotProtocol
                            . (cxt,ch,)
                        )
                )
                ( maybeToList $
                    def
                      ^? _CoprotocolDefn
                      % to
                        ( review _HPutExpectedOutputPolarityChToHaveProtocolButGotCoprotocol
                            . (cxt,ch,)
                        )
                )
          ...
          let (ttypeargs, unwrappedtp) = fromJust $ clauselkuptp ^? _SymConcPhrase
              ...
              eqn =
                TypeEqnsExist ([ttypepch'] <> ttypepargs) $
                  [ TypeEqnsEq
                      ( typePtoTypeVar ttypepch,
                        ttypepclause
                      ),
        ...
        CSplit cxt ch (ch0, ch1) -> do
          ...
          let ch' = _ChIdentT # (ch, fromJust $ lookupInferredSeqTypeExpr ttypech ttypemap)
              ...
              eqn =
                TypeEqnsExist [ttypepch0, ttypepch1] $
                  [ TypeEqnsEq
                      ( typePtoTypeVar ttypepch,
                        inputOutput
                          (ch ^. polarity)
              ...
              for allphrases $
                \(cxt, (ins, outs), cmds) -> do
                  let insr = map (view identR) ins
                      ...
                      pluggedins =
                        map (review _ChIdentR . (,Input)) $
                          filter (\identr -> identr `elem` insr) plugs
                      pluggedouts =
                        map (review _ChIdentR . (,Output)) $
                  ...
                  lift $
                    tell $
                      review _ExternalError $
                        foldMap expectedInputPolarity ins
        ...
        CCase cxt expr cases -> do
          ...
          let ttypepexpr = annotateTypeTag ttypeexpr expr
              ...
              eqn =
                TypeEqnsExist (ttypepexpr : ttypesppatts) $
        ...
        CSwitch cxt switches -> do
          ...
          let ttypepexprs = annotateTypeTags ttypeexprs (map fst $ NE.toList switches)
              eqn =
                TypeEqnsExist ttypepexprs $
                  zipWith
                    (\ann -> TypeEqnsEq . (,_TypeBoolF # Just (TypeAnnExpr $ ann)) . typePtoTypeVar)
                    (map fst $ NE.toList switches)
                    ttypepexprs
                    <> concat eqns
