...
instance BnfcKeyword B.Use where
  bnfcKeyword = B.Use ((-1, -1), "use")

instance BnfcKeyword B.StoreType where
  bnfcKeyword = B.StoreType ((-1, -1), "Store")

instance BnfcKeyword B.Store where
  bnfcKeyword = B.Store ((-1, -1), "store")
      ...
      f (TypeBuiltIn n) = case n of
        ...
        TypeTupleF cxt (t0, t1, ts) ->
          B.MPL_TUPLE_TYPE
            ...
            bnfcKeyword
        TypeStoreF _cxt tp -> case tp of
          (TypeBuiltIn (TypeConcArrF _ ss is os)) -> B.MPL_STORE_TYPE bnfcKeyword bnfcKeyword (B.PROCESS_TYPE (map f ss) (map f is) (map f os)) bnfcKeyword
          _ -> B.MPL_STORE_TYPE bnfcKeyword bnfcKeyword (B.PROCESS_TYPE [] [] []) bnfcKeyword
...
instance UserProvidedTypeToBnfc (Maybe ([MplType MplParsed], [MplType MplParsed], [MplType MplParsed])) MplParsed where
  ...
  userProvidedTypeToBnfc proxy (Just (seqs, ins, outs)) =
    Just $
...
instance UserProvidedTypeToBnfc (Maybe ([MplType MplParsed], MplType MplParsed)) x where
  userProvidedTypeToBnfc proxy Nothing = Nothing
  userProvidedTypeToBnfc proxy (Just (froms, to)) =
    Just $
      B.MPL_SEQ_ARROW_TYPE [] (map (mplTypeToBnfc proxy) froms) (mplTypeToBnfc proxy to)

instance
  (PPrint ident x) =>
  UserProvidedTypeToBnfc
    (Maybe ([ident], [MplType MplRenamed], [MplType MplRenamed], [MplType MplRenamed]))
    x
  where
  userProvidedTypeToBnfc proxy Nothing = Nothing
  userProvidedTypeToBnfc proxy (Just (foralls, seqs, ins, outs)) =
    Just $
...
instance
  (PPrint ident x) =>
  UserProvidedTypeToBnfc (Maybe ([ident], [MplType MplRenamed], MplType MplRenamed)) x
  where
  userProvidedTypeToBnfc proxy Nothing = Nothing
  userProvidedTypeToBnfc proxy (Just (foralls, froms, to)) =
    Just $
...
instance (PPrint ident x) => UserProvidedTypeToBnfc ([ident], [MplType MplTypeChecked], MplType MplTypeChecked) x where
  userProvidedTypeToBnfc proxy (foralls, froms, to) =
    Just $
      B.MPL_SEQ_ARROW_TYPE
        (map (toBnfcIdent proxy) foralls)
        (map (mplTypeToBnfc proxy) froms)
...
instance (PPrint ident x) => UserProvidedTypeToBnfc ([ident], [MplType MplTypeChecked], [MplType MplTypeChecked], [MplType MplTypeChecked]) x where
  userProvidedTypeToBnfc proxy (foralls, seqs, ins, outs) =
    Just $
      B.MPL_CONC_ARROW_TYPE
        (map (toBnfcIdent proxy) foralls)
        (map (mplTypeToBnfc proxy) seqs)
  ...
  mplPattToBnfc proxy = f
    where
      ...
      f (PSimpleConstructor _ id args) =
        B.CONSTRUCTOR_PATTERN_ARGS
          (toBnfcIdent proxy id)
          bnfcKeyword
...
instance (PPrint x y) => MplSimpleConstructorArgsToBnfc [(x, z)] y where
  mplSimpleConstructorArgsToBnfc proxy args =
...
instance
  ...
  MplPhraseToBnfc x (SeqObjTag DataDefnTag) B.SeqTypePhraseDefn y
  where
  mplPhraseToBnfc proxy phrase =
    B.SEQ_TYPE_PHRASE
      [phrase ^. typePhraseName % to (toBnfcIdent proxy)]
...
instance
  ...
  MplPhraseToBnfc x (SeqObjTag CodataDefnTag) B.SeqTypePhraseDefn y
  where
  mplPhraseToBnfc proxy phrase =
    B.SEQ_TYPE_PHRASE
      [phrase ^. typePhraseName % to (toBnfcIdent proxy)]
...
instance
  ...
  MplPhraseToBnfc x (ConcObjTag ProtocolDefnTag) B.ConcurrentTypePhraseDefn y
  where
  mplPhraseToBnfc proxy phrase =
    B.CONCURRENT_TYPE_PHRASE
      [phrase ^. typePhraseName % to (toBnfcIdent proxy)]
...
instance
  ...
  MplPhraseToBnfc x (ConcObjTag CoprotocolDefnTag) B.ConcurrentTypePhraseDefn y
  where
  mplPhraseToBnfc proxy phrase =
    B.CONCURRENT_TYPE_PHRASE
      [phrase ^. typePhraseName % to (toBnfcIdent proxy)]
...
instance
  ...
  MplClauseToBnfc x (SeqObjTag DataDefnTag) B.SeqTypeClauseDefn y
  where
...
instance
  ...
  MplClauseToBnfc x (SeqObjTag CodataDefnTag) B.SeqTypeClauseDefn y
  where
  mplClauseToBnfc proxy clause =
...
instance
  ...
  MplClauseToBnfc x (ConcObjTag ProtocolDefnTag) B.ConcurrentTypeClauseDefn y
  where
  mplClauseToBnfc proxy clause =
...
instance
  ...
  MplClauseToBnfc x (ConcObjTag CoprotocolDefnTag) B.ConcurrentTypeClauseDefn y
  where
  mplClauseToBnfc proxy clause =
...
instance
  ...
  mplExprToBnfc proxy = f
    where
      ...
      f (EBool _ id) =
        B.DESTRUCTOR_CONSTRUCTOR_ARGS_EXPR
      ...
      f (EStore _ (Left id)) =
        B.STORE_EXPR bnfcKeyword bnfcKeyword (B.PROCESS_N $ toBnfcIdent proxy id) bnfcKeyword
      f (EStore _ (Right ((patts, ins, outs), cmds))) =
        B.STORE_EXPR
          bnfcKeyword
          bnfcKeyword
          ( B.PROCESS_P
...
instance
  ...
  MplCmdToBnfc (MplCmd x) y
  where
  mplCmdToBnfc proxy = f
    where
      f (CRun _ call seqs ins outs) =
        let call' = case call of
              Left id -> B.PROCESS_NAME (toBnfcIdent proxy id)
              Right expr -> B.PROCESS_USE bnfcKeyword bnfcKeyword (mplExprToBnfc proxy expr) bnfcKeyword
         in B.PROCESS_RUN
              call'
              bnfcKeyword
              (map (mplExprToBnfc proxy) seqs)
              (map (toBnfcIdent proxy) ins)
              (map (toBnfcIdent proxy) outs)
              bnfcKeyword
      ...
      f (CCase cxt caseon cases) =
        B.PROCESS_CASE
          bnfcKeyword
          (mplExprToBnfc proxy caseon)
          $ NE.toList
          $ fmap g cases
        where
          g (patt, cmds) =
            B.PROCESS_CASE_PHRASE
              (mplPattToBnfc proxy patt)
              (mplCmdsToBnfc proxy cmds)
      f (CSwitch cxt switches) = B.PROCESS_SWITCH $ NE.toList $ fmap g switches
        where
          g (expr, cmds) =
            B.PROCESS_SWITCH_PHRASE
      ...
      f (CIllegalInstr _) =
        B.PROCESS_RUN (B.PROCESS_NAME (toBnfcIdent proxy "ILLEGALINSTR")) bnfcKeyword [] [] [] bnfcKeyword
...
instance (PPrint ident y, MplCmdToBnfc t y) => MplToForkPhrase (ident, [ident], NonEmpty t) y where
  mplToForkPhrase proxy (ch, cxt, cmds) =
    B.FORK_WITH_PHRASE
      (toBnfcIdent proxy ch)
      (map (toBnfcIdent proxy) cxt)
      $ mplCmdsToBnfc proxy cmds
...
where
  ...
  -}
instance
  (PPrint ident y, MplCmdToBnfc cmd y) =>
  MplToPlugPhrase ((), ([ident], [ident]), NonEmpty cmd) y
  where
  mplToPlugPhrase proxy (cxt, (ins, outs), cmds) =
    B.PLUG_PHRASE_AS
      (map (toBnfcIdent proxy) ins)
      (map (toBnfcIdent proxy) outs)
      $ mplCmdsToBnfc proxy cmds
...
mplStmtToBnfc proxy (MplStmt defns wheres) =
  B.MPL_DEFN_STMS_WHERE
    (NE.toList $ fmap (mplDefnToBnfc proxy) defns)
