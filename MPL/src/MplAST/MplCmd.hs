...
-- data PCall x = PName (IdP x) | PUse (XMplExpr x)

-- deriving instance (Show (IdP x), Show (XMplExpr x)) => Show (PCall x)

-- data ProcessCommand pattern letdef typedef seqcalleddef conccalleddef ident chident =
data MplCmd x
  = CRun !(XCRun x) (Either (IdP x) (XMplExpr x)) [XMplExpr x] [ChP x] [ChP x]
