...
testRunParser pa inp = runParser pa "" inp
...
indentBlock playoutword popenbrace pclosebrace pindentline psep = do
  ...
  where
    ...
    pindentlines = do
        indentline <- pindentlines' <|> pindentline 
        indentlines <- P.many pindentlines'
    ...
    {- TODO: need to fix the following parse issues:
        ...
        do
            ...
            b
        @
        will error -- to fix, we need to relax the indentation after 
        a semicolon is parsed (I think in the original paper this is an
        error as well).
    ...
    -}
    pindentlines' = P.many1 (indentGt psep) >> pindentline 
