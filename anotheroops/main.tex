...
\textbf{problem number 1 -- the first negation that is executed}
...
\end{verbatim}
(everything past here is mostly for fun and further understanding 
    of what the machine is actually doing)
Now, let's let \verb|p2_bot_bot| take a step (note that this too can never suceed taking a step on the channel because of problem 1). 
\begin{verbatim}
-- Channel manager
( _inpterm0,  
    []
...
Now, we let \verb|p1_bot| take a step (note that this just executes \verb|p1|, so we substitute arguments as needed).
\begin{verbatim}
-- Channel manager
( _inpterm0,  
    []
...
\end{verbatim}
Now, we let \verb|p1| take a step.
\begin{verbatim}
-- Channel manager
( _inpterm0,  
    []
...
\end{verbatim}
Another step for \verb|p1|. Note that since we have a ``split'',
    we generate some new fresh names for the splitted channels, say \verb|mm0,nmpp0|
    and we substitue as necessary.
Moreover, we note that we modify the channel translations of \verb|p1|
\begin{verbatim}
-- Channel manager
( _inpterm0,  
    []
...
Another step for \verb|p1| (note it suspends from the ``get'' command)
\begin{verbatim}
-- Channel manager
( _inpterm0,  
    []
...
-- Running processes:
\end{verbatim}
A step for the channel \verb|pp|.
\begin{verbatim}
-- Channel manager
( _inpterm0,  
    []
...
-- Running processes:
\end{verbatim}
But now the system is in a complete deadlock.. Oops!
