...
defn
    -- tree node with a client (given to racer node) and two sub trees (given to racer_tree_node) that are all raced
    proc racer_tree_client_node :: [Char] | Put(NewMsg | RecvMsgs), Put(FwdMsg | Link), Put(FwdMsg | Link) => Put(FwdMsg | Link) =
        node_id | client, tree_1, tree_2 => root -> plug
            tree_1, tree_2 => sub_trees -> race
                tree_1 -> racer_tree_node(node_id | tree_1, tree_2 => sub_trees)
                tree_2 -> racer_tree_node(node_id | tree_2, tree_1 => sub_trees)
            racer_node(node_id | client, sub_trees => root)
    ...
    --             case rec of							
    --                 Yes(msg) -> do
    --                     put Msg(Yes(msg), "Trace: " ++ node_id) on root   -- root now Link
    --                     hcase root of
    --                         Fwd -> do               -- root now Put(FwdMsg | Link)
    --                             hput Recv on client -- client now Put(NewMsg | RecvMsgs)
    ...
    --                     hput Close on client
    --                     close client
    --                     put Msg(End(cid), "Trace: " ++ node_id) on root      -- root now Link
    ...
    --         case rec of			
    --             Msg(msg, trace) -> do
    --                 put Msg(msg, trace ++ ", " ++ node_id) on root            -- root now Link
    --                 hcase root of
    --                     Fwd -> do                   -- root now Put(FwdMsg | Link)
    --                         hput Fwd on tree_1      -- tree_1 now Put(FwdMsg | Link)
    --                         racer_tree_client_node(node_id | client, tree_1, tree_2 => root)
