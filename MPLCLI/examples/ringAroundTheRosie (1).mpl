...
proc p2 :: | Passer (| MemCell ( Int |)) => Passer (| MemCell ( Int |)) =
    | passer3 => passer1 ->
        hcase passer3 of
            Passer -> do
            ...
            PasserTerminate -> do
                close passer3
                hput PasserTerminate on passer1
                halt passer1
...
proc p3 :: | Get([Char] | TopBot) => Passer ( | MemCell (Int|)) =
    | ch => passer -> do
            ...
            npasser -> 
                if (i + 1 == 3000) 
                    then do
                        put "END" on ch
                        close ch
                        hput PasserTerminate on npasser
                        halt npasser
                    else
                        p3 ( | ch => npasser )
...
proc run :: | Console =>  =
    | console => -> do
        ...
        plug
            ch => ->
                plug
                    memCell ( 0 | ch0 => ) --the memory cell starts with the empty string.
                    p1 ( | ch1 => ch0)
                    p2 ( | ch2 => ch1)
                    p3 ( | ch => ch2)
            console => ch -> do
