...
`lib/`
Directory of Haskell source code shared across projects.
 - `lib/ServiceConstants.hs` contains the "magic" constants used by services to communicate properly between them
 - `lib/Data/Queue.hs` is an implementation of a Queue based of Chris Okasaki's work
 - `lib/Data/Stream.hs` is an implementation of a Stream (infinite list).
...
`package.yaml`
A file used by stack (hpack) to generate the  `.cabal`. Edit this file to manage dependencies.
...
Directory contain Haskell source code for the automated testing of parts of the project.
  - `tsts/QueueTests.hs` contains code to test the correctness of the queue.
