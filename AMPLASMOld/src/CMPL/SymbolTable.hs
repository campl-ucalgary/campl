...
data SYM_ENTRY =  S_HAND   [(String,Int)]
                ...
                | S_CONSTR [(String,Int,Int)] 
                    -- Constructor name, sum type number, number of args
                    -- (there are multiple constructors)
                | S_DESTR  [(String,Int,Int)]
                    -- Destructor name, sum type number, number of args
                    -- (there are multiple constructors)
                | S_FUN Int
                    -- Number of args
                | S_PROC Int (Int,Int)
                    -- number of sequential args,
...
collect_symbols ( AMPLcode hs chs cs ds ps fs _ ) = M.fromList st
     where
      st = 
        ...
        . collect_processes ps
        . collect_destructors ds
        . collect_constructors cs
        . collect_cohandles chs
        . collect_handles hs $ []
--------------------------------------------------------------------------------------
